...
function processData() {
    ...
    appData.userSelections.forEach(user => {
        ...
        // Get all selected players (adjust property names as needed)
        ...
        selectedPlayers.forEach(playerName => {
            // Find player by name (consider adding PlayerId to selections if possible)
            const player = Object.values(playerMap).find(p => p.name === playerName);
            if (player) {
                const playerId = Object.keys(playerMap).find(id => playerMap[id].name === playerName);
